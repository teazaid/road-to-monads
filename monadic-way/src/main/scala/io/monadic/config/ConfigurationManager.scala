...
class ConfigurationManager {
  type ConfigAction[T] = Reader[Config, Try[T]]

  def getConfig(): ConfigAction[AppConfig] = Reader { config =>
    for {
      confirmationSettings <- getConfigPath("confirmation").run(config)
      dbSettings <- getConfigPath("db").run(config)
  ...
  private def getConfigPath(path: String): ConfigAction[Config] = Reader {config =>
